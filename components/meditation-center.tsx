...
export function MeditationCenter({ sessions, onAddSession }: MeditationCenterProps) {
  ...
  }, [isRunning, timeLeft, phase, currentCycle, activeExercise, sessionStartTime, onAddSession])
  ...
  const startGuidedMeditation = (type: MeditationSession["type"], duration: number) => {
    onAddSession({
      type,
      duration,
      completedAt: new Date(),
  ...
  const averageRating =
    sessions.length > 0 ? sessions.reduce((sum, s) => sum + (s.rating || 0), 0) / sessions.length : 0
          ...
          <CardContent>
            {activeExercise ? (
              <div className="text-center space-y-4">
                ...
                <div className="text-lg capitalize text-muted-foreground">{phase}</div>
                <Progress value={((currentCycle + 1) / activeExercise.cycles) * 100} className="w-full" />
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="flex gap-2 mt-2">
                            <Badge variant="outline">{exercise.inhale}s inhale</Badge>
                            {exercise.hold > 0 && <Badge variant="outline">{exercise.hold}s hold</Badge>}
                            <Badge variant="outline">{exercise.exhale}s exhale</Badge>
                          </div>
                        </div>
                        <Button onClick={() => startBreathingExercise(exercise)} size="sm">
          ...
          <CardContent>
            <div className="space-y-4">
              {guidedMeditations.map((meditation) => (
                <Card key={meditation.type} className="hover:shadow-md transition-shadow">
                  <CardContent className="p-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {meditation.durations.map((duration) => (
                            <Button
                              ...
                              size="sm"
                              variant="outline"
                              onClick={() => startGuidedMeditation(meditation.type, duration)}
                            >
                              {duration} min
                            </Button>
                          ))}
